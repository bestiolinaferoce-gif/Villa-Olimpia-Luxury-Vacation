    ...
    <div className="min-h-screen pt-20">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-ocean/15 via-primary/10 to-amber-50 py-16">
        ...
        <div className="container relative mx-auto px-4">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 text-lg text-slate-600 leading-relaxed">{copy.hero.intro}</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              ...
              <Button variant="outline" size="lg" asChild>
                <Link href={prenotaHref}>{copy.hero.ctaSecondary}</Link>
              </Button>
            </div>
            <p className="mt-3 text-sm text-slate-500">{copy.hero.note}</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-slate-50/80 border-y border-primary/10">
        <div className="container mx-auto px-4 max-w-4xl">
          <h2 className="text-3xl font-playfair font-bold text-slate-900 mb-6 text-center">{copy.idealTitle}</h2>
          <div className="grid md:grid-cols-3 gap-6">
            {copy.ideal.map((item, idx) => {
              const Icon = [Heart, Users, Globe][idx] ?? Heart
              return (
                <Card key={item.title}>
                  <CardHeader>
                    <Icon className="h-6 w-6 text-primary mb-2" />
                    <CardTitle className="text-lg font-playfair">{item.title}</CardTitle>
      ...
      <section className="py-14 bg-white border-y border-primary/10">
        <div className="container mx-auto max-w-3xl px-4">
          <div className="rounded-2xl border-2 border-primary/12 bg-gradient-to-br from-white via-primary/[0.03] to-ocean/[0.06] p-8 text-center shadow-sm">
            <h2 className="font-playfair text-2xl font-bold text-slate-900 md:text-3xl">{copy.band.title}</h2>
            <p className="mt-2 text-slate-600">{copy.band.text}</p>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
              ...
              <Button variant="luxury" size="lg" asChild>
                <Link href={contattiBandHref}>{copy.band.form}</Link>
              </Button>
            </div>
          </div>
        </div>
      ...
      <SeasonalAvailabilityGrid config={config} monthKey={monthKey} locale={locale} />
      <SeasonalUrgencyForm
        config={config}
        monthKey={monthKey}
        ...
        locale={locale}
      />
      <SeasonalFAQ month={monthKey} locale={locale} />
      ...
      <section className="py-16 bg-gradient-to-br from-primary/5 to-ocean/5">
        <div className="container mx-auto max-w-2xl px-4 text-center">
          ...
          <p className="mb-8 text-slate-600">{copy.closing.text}</p>
          <div className="flex flex-col justify-center gap-3 sm:flex-row">
            <Button variant="luxury" size="lg" asChild>
              <Link href={appartamentiHref}>
                {copy.closing.apartments}
                <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
            </Button>
            <Button variant="outline" size="lg" asChild>
              <Link href={homeHref}>{copy.closing.home}</Link>
            </Button>
          </div>
        </div>
      </section>
