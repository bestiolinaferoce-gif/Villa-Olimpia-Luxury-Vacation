...
const DEFAULT_SHARE_KIT_COPY: ShareKitCopy = {
...
export function ShareKit({
  ...
  const c = { ...DEFAULT_SHARE_KIT_COPY, ...copy }
  const [copied, setCopied] = useState(false)

  const shareUrl = `${SITE_CONFIG.url}${path.startsWith("/") ? path : `/${path}`}?source=referral`
  ...
  return (
    <div className="rounded-2xl border border-primary/15 bg-gradient-to-br from-amber-50 via-white to-sky-50 p-6 shadow-lg">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-xl font-playfair font-bold text-slate-900">{c.title}</h3>
      </div>
      <p className="mt-2 text-sm text-slate-600">{c.description}</p>

      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-700">
        {c.shareText} <span className="text-primary">{shareUrl}</span>
      </div>

      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Button asChild variant="luxury" className="w-full">
          <a href={buildWhatsAppUrl(c.shareText, shareUrl)} target="_blank" rel="noopener noreferrer">
        ...
        <Button variant="outline" className="w-full" onClick={handleCopy}>
          ...
          {copied ? c.copiedText : c.copyText}
        </Button>
      </div>
    </div>
  )
